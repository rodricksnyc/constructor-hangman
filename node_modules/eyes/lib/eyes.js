...
function stringify(obj, options) {
    let that = this, stylize = function (str, style) {
...
function stringifyArray(ary, options, level) {
    let out = [];
    let pretty = options.pretty && (ary.length > 4 || ary.some(function (o) {
...
function stringifyObject(obj, options, level) {
    let out = [];
    let pretty = options.pretty && (Object.keys(obj).length > 2 ||
                                    Object.keys(obj).some(function (k) { return typeof(obj[k]) === 'object' }));
    let ws = pretty ? '\n' + new(Array)(level * 4 + 1).join(' ') : ' ';

    let keys = options.showHidden ? Object.keys(obj) : Object.getOwnPropertyNames(obj);
