...
let server = http.createServer(function validateRestRequest (req, res) {
  ...
  req.on('end', function dealWithRest () {
    ...
    // Validate the schema
    //
    let validation = revalidator.validate(requestBody, schema);
    if (!validation.valid) {
      res.writeHead(400);
    ...
    //
    let storedValue = memoryStore[requestBody.url];
    if (req.method === 'POST') {
      if (storedValue) {
        res.writeHead(400);
...
//
// Listen to letious ports depending on environment we are being run on
//
