...
let stdin, stdout, history = [];
let prompt = module.exports = Object.create(events.EventEmitter.prototype);
let logger = prompt.logger = new winston.Logger({
...
prompt.history = function (search) {
  ...
  let names = history.map(function (pair) {
...
// #### @schema {Array|Object|string} Set of letiables to get input for.
...
prompt.confirm = function (/* msg, options, callback */) {
  let args     = Array.prototype.slice.call(arguments),
      ...
      callback = args.pop(),
      opts     = args.shift(),
      lets     = !Array.isArray(msg) ? [msg] : msg,
      RX_Y     = /^[yt]{1}/i,
      RX_YN    = /^[yntf]{1}/i;

  function confirm(target, next) {
    let yes = target.yes || RX_Y,
      options = utile.mixin({
  ...
  async.rejectSeries(lets, confirm, function(result) {
    callback(null, result.length===0);
  });
};


// letiables needed outside of getInput for multiline arrays.
let tmp = [];


// ### function getInput (prop, callback)
// #### @prop {Object|string} letiable to get input for.
// #### @callback {function} Continuation to pass control to when complete.
// Gets input from the user via stdin for the specified message `msg`.
//
prompt.getInput = function (prop, callback) {
  let schema = prop.schema || prop,
  ...
  let type = (schema.properties && schema.properties[propName] &&
  ...
  read({
    ...
    if (line !== '') {
      ...
      if (schema.properties[propName]) {
        let type = (schema.properties[propName].type || '').toLowerCase().trim() || undefined;
        ...
        //
        if (type == 'array') {
          let length = prop.schema.maxItems;
          if (err) {
...
function convert(schema) {
  let newProps = Object.keys(validate.messages),
      newSchema = false,
      key;
