...
Get stdout window width, with four fallbacks, `tty`, `output.columns`, a custom environment letiable and then a default.
...
let cliWidth = require('cli-width');

cliWidth(); // maybe 204 :)
```

You can also set the `CLI_WIDTH` environment letiable.

If none of the methods are supported, and the environment letiable isn't set,
