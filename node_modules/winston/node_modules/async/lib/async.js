...
(function () {
    ...
    let _forEachOf = function (object, iterator) {
    ...
    let _baseSlice = function (arr, start) {
        start = start || 0;
        let index = -1;
    ...
    async.eachLimit = function (arr, limit, iterator, callback) {
        let fn = _eachLimit(limit);
        fn.apply(null, [arr, iterator, callback]);
    };
    async.forEachLimit = async.eachLimit;

    let _eachLimit = function (limit) {
    ...
    async.forEachOf = async.eachOf = function (object, iterator, callback) {
        ...
        let size = object.length || _keys(object).length;
        let completed = 0;
        if (!size) {
            return callback();
        }
    ...
    let _forEachOfLimit = function (limit) {
    ...
    let doParallel = function (fn) {
        return function () {
            let args = _baseSlice(arguments);
            return fn.apply(null, [async.each].concat(args));
        };
    };
    let doParallelLimit = function(limit, fn) {
        return function () {
            let args = _baseSlice(arguments);
    ...
    async.auto = function (tasks, callback) {
        ...
        _each(keys, function (k) {
            ...
            let taskCallback = function (err) {
                let args = _baseSlice(arguments, 1);
                if (args.length <= 1) {
                    args = args[0];
                }
                if (err) {
    ...
    async.retry = function(times, task, callback) {
        ...
        let wrappedTask = function(wrappedCallback, wrappedResults) {
            let retryAttempt = function(task, finalAttempt) {
    ...
    let _parallel = function(eachfn, tasks, callback) {
        callback = callback || noop;
        if (_isArray(tasks)) {
        ...
        return function () {
            ...
            async.reduce(fns, args, function (newargs, fn, cb) {
                fn.apply(that, newargs.concat([function () {
                    let err = arguments[0];
                    let nextargs = _baseSlice(arguments, 1);
                    cb(err, nextargs);
                }]));
            },
            function (err, results) {
    ...
    let _applyEach = function (eachfn, fns /*args...*/) {
        let go = function () {
            let that = this;
            let args = _baseSlice(arguments);
            let callback = args.pop();
        ...
        if (arguments.length > 2) {
            let args = _baseSlice(arguments, 2);
