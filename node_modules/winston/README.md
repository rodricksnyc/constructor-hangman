...
The way these objects are stored leties from transport to transport (to best support the storage mechanisms offered). Here's a quick summary of how each transports handles metadata:
...
let infoFile = logger.transports[0];
...
``` js
  ...
  let logger = new (winston.Logger)({ exitOnError: ignoreEpipe });
...
``` js
  ...
  //
  let category1 = winston.loggers.get('category1');
