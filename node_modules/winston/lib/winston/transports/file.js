...
//
let File = exports.File = function (options) {
  let self = this;
  ...
  //
  // Internal state letiables representing the number
...
File.prototype.query = function (options, callback) {
  ...
  let file = path.join(this.dirname, this.filename),
  ...
  (function checkFile (target) {
    ...
    function compressFile() {
      if (self._archive) {
        ...
        let inp = fs.createReadStream(String(self._archive));
        let out = fs.createWriteStream(self._archive + '.gz');
...
File.prototype._incFile = function (callback) {
  let ext = path.extname(this._basename),
...
File.prototype._checkMaxFilesTailable = function (ext, basename, callback) {
  ...
  for (let x = this.maxFiles - 1; x > 0; x--) {
    tasks.push(function (i) {
      return function (cb) {
        let tmppath = path.join(self.dirname, basename + (i - 1) + ext +
          (self.zippedArchive ? '.gz' : ''));
