...
//
let Logger = exports.Logger = function (options) {
...
Logger.prototype.log = function (level) {
  ...
  //
  let callback  = typeof args[args.length - 1] === 'function'
    ? args.pop()
    : null;
  ...
  //
  let targets = this._names.filter(function (name) {
    let transport = self.transports[name];
  ...
  //    logger.info('No interpolation symbols', 'ok', 'why', { meta: 'is-this' });
  //
  let metaType  = Object.prototype.toString.call(args[args.length - 1]),
  ...
  this.filters.forEach(function(filter) {
    let filtered = filter(level, msg, meta, self);
...
Logger.prototype.stream = function (options) {
  ...
  if (options.transport) {
    let transport = this.transports[options.transport];
  ...
  transports.forEach(function (transport) {
    let stream = transport.stream(options);
    if (!stream) return;
...
Logger.prototype.unhandleExceptions = function () {
  ...
  if (this.catchExceptions) {
    Object.keys(this.exceptionHandlers).forEach(function (name) {
      let handler = self.exceptionHandlers[name];
      if (handler.close) {
        handler.close();
      }
    });
...
Logger.prototype.add = function (transport, options, created) {
  let instance = created ? transport : (new (transport)(options));
...
Logger.prototype.remove = function (transport) {
  let name = typeof transport !== 'string'
  ...
  let instance = this.transports[name];
...
Logger.prototype.profile = function (id) {
  ...
  if (this.profilers[id]) {
    ...
    // Support letiable arguments: msg, meta, callback
