...
module.exports = function (transport, options) {
  let logger = transport instanceof winston.Logger
    ? transport
    : new winston.Logger({
        transports: [
  ...
  let common = require('../../lib/winston/common');
  common.timestamp = function() {
    return new Date().toISOString();
  };

  let transport = logger.transports[logger._names[0]];
      ...
      'the query() method': {
        ...
        'using the `from` and `until` option': {
          'topic': function () {
            if (!transport.query) return;
            let cb = this.callback;
            let start = Date.now() - (100 * 1000);
            let end = Date.now() + (100 * 1000);
